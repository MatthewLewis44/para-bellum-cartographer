...
7. Rivers (AD-026 node model): `has_river` + `river_name` (a significant river
   ∩ the hex polygon) from the AD-011 filtered set; `river_edges` retained as a
   render-direction hint; bridges, ports
...
- **River node migration (P0-A, AD-026)**: rivers are now the hexes a river
  *passes through* (`has_river` = a significant AD-011 river ∩ the hex polygon),
